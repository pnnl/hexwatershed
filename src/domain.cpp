...
int domain::domain_delineate_watershed()
{
  ...
  while (iFlag_finished == 0)
  {
    ...
    if (vCell_dummy2.size() > 0)
    {
      aIndex = find_lowest_cell(vCell_dummy2);
      double dElevation_center = vCell_active.at(aIndex[1]).dElevation;
      vCell_dummy2.erase(vCell_dummy2.begin() + aIndex[1]);
      //set flag
      vFlag.at(aIndex[0]) = 1;
      //loop through

      for (int i = 0; i < vCell_active.at(aIndex[0]).nNeighbor; i++)
      {
        long lIndex_dummy = vCell_active.at(aIndex[0]).aNeighbor[i];
        if (vFlag[lIndex_dummy] == 1)
        {
          //already removed?
        }
        else
        {
          double dElevation_dummy = vCell_active[lIndex_dummy].dElevation;
          if (dElevation_dummy < dElevation_center)
          {
            vCell_active[lIndex_dummy].dElevation = dElevation_center;
            vFlag[lIndex_dummy] = 1;

            //push on to queue

            vCell_dummy2.push_back(vCell_active[lIndex_dummy]);
          }
        }
      }
    }
...
std::array<long, 2> domain::find_lowest_cell(std::vector<cell> vCell_in)
{
  ...
  else
  {
    ...
    for (iIterator1 = vCell_in.begin(); iIterator1 != vCell_in.end(); iIterator1++)
    {
      if ((*iIterator1).dElevation < dElevation_lowest)
      {
       ...
       lIndex_lowest = (*iIterator1).lGlobalID;
      }
      else
      {
        continue;
    ...
    //be careful
    aIndex_out.at(0) = vCell_in.at(lIndex_lowest).lGlobalID;
    aIndex_out.at(1) = lIndex_lowest;
  }
  return aIndex_out;
}
  ...
  for (lIndex_self = 0; lIndex_self < vCell_active.size(); lIndex_self++)
  {
    ...
    for (lIndex1 = 0; lIndex1 < vCell_active.size(); lIndex1++)
    {
      ...
      else
      {
        ...
        dDistance = sqrt((dDummy_X - dX_self) * (dDummy_X - dX_self) + (dDummy_Y - dY_self) * (dDummy_Y - dY_self));
    ...
    for (lIndex2 = 0; lIndex2 < vCell_active.size(); lIndex2++)
    {
      ...
      else
      {
        ...
        {
          //found one
          (vCell_active.at(lIndex_self)).vNeighbor.push_back((vCell_active.at(lIndex2)).lID);
