...
* It can provide continental to global coverage at consistent or variable spatial resolutions [@Renssen2000]. It can be used to couple land surface/hydrologic models with oceanic models using a unified mesh grid (e.g., the Voronoi tessellation of the Model for Prediction Across Scale (MPAS)) [@Ringler2013; @Sahr2019].
...
We have developed a watershed delineation model (HexWatershed) using the hexagon grid spatial discretization method. We have applied this model to a watershed in the western US. Model outputs have shown that HexWatershed can reproduce all the watershed delineation characteristics.
